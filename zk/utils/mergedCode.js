...
export class ZkCredential {
  ...
  static #performModularReduction(rawBigInt) {
    return ((rawBigInt % this.#FIELD_PRIME) + this.#FIELD_PRIME) % this.#FIELD_PRIME;
  }
  ...
  static async #applyERC721Gating(tokenAddress, privateKey, alchemyUrl) {
    ...
    if (tokenAddress && privateKey && alchemyUrl) {
            ...
            throw new Error("ERC721_ADDRESS, PRIVATE_KEY or URL not properly set. ERC721 check cannot be performed.");
        }
  }
...
(async () => {
  ...
  const { identity, commitment, mnemonic } =
    await ZkCredential.generateCredentials(
        /*{erc721Gating: { 
            tokenAddress: process.env.ERC721_ADDRESS,
            privateKey: process.env.PRIVATE_KEY,
            alchemyUrl: process.env.ALCHEMY_SEPOLIA_URL
        }}*/
    );
