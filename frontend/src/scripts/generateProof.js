...
export class ProofGenerator {
    ...
    static #MEMBERSHIP_WASM_PATH = "../zk/circuits/membership/build/membership_circuit_js/membership_circuit.wasm";
    static #MEMBERSHIP_ZKEY_PATH = "../zk/circuits/membership/build/membership_circuit_final.zkey";
    static #MEMBERSHIP_VKEY_PATH = "../zk/circuits/membership/build/membership_circuit_key.json";
    ...
    static async filterLeavesByGroupId(leavesJson, groupId) {
        ...
        const commitmentArray = filteredLeaves.map(leaf => BigInt(leaf.commitment_value));
    ...
    static async generateCircuitInput(mnemonic, commitmentArray) {
        ...
        const { trapdoor, nullifier, commitment } = await ZkCredential.generateIdentity(trapdoorKey, nullifierKey);

        const index = commitmentArray.findIndex(leaf => leaf === commitment);

        const { root, pathElements, pathIndices } = await MerkleTreeService.generateMerkleProof(index, commitmentArray);
     ...
     */
    static async verifyProofOffChain(proof, publicSignals, circuitType = "membership") {
    ...
    static async verifyProofOnChain(proof, publicSignals, contract) {
        try {
            const isValid = await contract.verifyMembershipProof(proof, publicSignals);
