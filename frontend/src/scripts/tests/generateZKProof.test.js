...
const proposalDescription =
  "Proposal to upgrade the Sonic Screwdriver to v42.0, adding quantum syncing, voice commands, and optional ice cream mode—keeping the Doctor stylishly overpowered across space and time.";
const proposalPayload =
  '{"value":"0","calldata":{"amount":"2","recipient":"0xabcdefabcdefabcdefabcdefabcdefabcdefabcd"},"target_action":"delegateDistributeGrant","target_contract":"0xGovernanceContractAddress","target_function":"executeProposal"}';
const proposalFunding =
  '{"type":"lumpsum payment","amount":"2","currency":"ETH"}';
const proposalMetadata =
  '{"ipfs_cid":"QmTwui9718uxaUmyHasXDQ63RVoZKTQSnmRZfhGPRV1bzX"}';
const voteChoice = 1;

const mnemonic_user1 =
  "forward fiction concert during pencil man boy slice slab engage knife smoke";

const users = [{ user: "test7@mail.com", mnemonic: mnemonic_user1 }];
...
for (const user of users) {
  ...
  const membershipCircuitInput =
    await ZKProofGenerator.generateMembershipCircuitInput(
      user.mnemonic,
      commitmentArray,
      groupId
    );

  const { proof: membeshipProof, publicSignals: membershipPublicSignals } =
    await ZKProofGenerator.generateProof(membershipCircuitInput, "membership");
  ...
  console.log(
    "Membership Solidity publicSignals: ",
    membershipPublicSignalsSolidity
  );
  ...
  console.log(
    `Is proof valid for ${user.user} and membership circuit:`,
  ...
  const proposalCircuitInput =
    await ZKProofGenerator.generateProposalCircuitInput(
      user.mnemonic,
      commitmentArray,
      groupId,
      epochId,
  ...
  const { proof: proposalProof, publicSignals: proposalPublicSignals } =
    await ZKProofGenerator.generateProof(proposalCircuitInput, "proposal");
  ...
  console.log(
    "Proposal Solidity publicSignals: ",
    proposalPublicSignalsSolidity
  );
  ...
  console.log("Verifying proof off-chain...");
  console.log(
    `Is proof valid for ${user.user} and proposal circuit:`,
    isValidProposalProof
  );
  ...
  const proposalClaimCircuitInput =
  ...
  console.log(
    "Proposal Claim Solidity publicSignals: ",
    proposalClaimPublicSignalsSolidity
  );
  ...
  console.log("Verifying proof off-chain...");
  console.log(
    `Is proof valid for ${user.user} and proposal claim circuit:`,
    isValidProposalClaimProof
  );
  ...
  const { proof: voteProof, publicSignals: votePublicSignals } =
    await ZKProofGenerator.generateProof(voteCircuitInput, "vote");
  ...
  console.log("Verifying proof off-chain...");
  console.log(
    `Is proof valid for ${user.user} and vote circuit:`,
    isValidVoteProof
  );
}
