 ...
 * including membership, vote, and proposal circuits.
...
export class ZKProofGenerator {
  ...
  // Vote circuit paths
  static get #VOTE_WASM_PATH() {
    return join(this.#getPublicDir(), "vote_circuit", "vote_circuit.wasm");
  }
  static get #VOTE_ZKEY_PATH() {
    return join(
      this.#getPublicDir(),
      "vote_circuit",
      "vote_circuit_final.zkey"
    );
  }
  static get #VOTE_VKEY_PATH() {
    return join(this.#getPublicDir(), "vote_circuit", "vote_circuit_key.json");
  }
  ...
  static async #getFieldModulusForBn128() {
    const bn128 = await buildBn128();
    const fieldModulus = bn128.Fr.p;
    console.log("FM: ", fieldModulus, "type: ", typeof fieldModulus);
    return fieldModulus;
  }
  ...
  static async filterLeavesByGroupId(leavesJson, groupId) {
    let data = readFileSync(leavesJson);
  ...
  static async generateMerkleProofInput(mnemonic, commitmentArray) {
    ...
    console.log("Nullifier Key:", nullifierKey);
    ...
    if (Array.isArray(input)) {
      ...
      inputArray = input.map((item) => {
        if (typeof item === "string") {
          return this.#moduloReduction(this.#hashStrToBigInt(item));
      ...
      if (typeof input === "string") {
        inputArray = [this.#moduloReduction(this.#hashStrToBigInt(input))];
