...
import { ProofGenerator } from "../generateProof.js";
...
const mnemonic_user1 = "immune common syrup eight obscure include cake wagon night bid orange blind"
// User2 test2@mail.com
const mnemonic_user2 = "spare find poverty envelope boost fiscal black spare zero ticket legend point"
// User3 test3@mail.com
const mnemonic_user3 = "service nice lava heavy patient satisfy fresh run index never castle dream";
...
const commitmentArray = await ProofGenerator.filterLeavesByGroupId(
...
async function testProof(mnemonic) {
    ...
    const circuitInput = await ProofGenerator.generateCircuitInput(mnemonic, commitmentArray);
    const { proof, publicSignals } = await ProofGenerator.generateProof(circuitInput, "membership");
    const isValidProof = await ProofGenerator.verifyProofOffChain(proof, publicSignals, "membership");
