...
async function precomputePoseidonConstants() {
    ...
    const abstainHash = poseidon([0n]);
    const yesHash = poseidon([1n]);
    const noHash = poseidon([2n]);

    console.log("Poseidon(0) (Abstain):", poseidon.F.toString(abstainHash));
    console.log("Poseidon(1) (Yes):", poseidon.F.toString(yesHash));
    console.log("Poseidon(2) (No):", poseidon.F.toString(noHash));
}
